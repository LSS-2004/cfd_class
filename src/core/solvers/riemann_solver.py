...
def exact_riemann_solution(q_left: np.ndarray, q_right: np.ndarray, g: float = 9.81) -> Dict:
    """
    浅水方程精确Riemann求解器
...
def godunov_flux(q_left: np.ndarray, q_right: np.ndarray, g: float = 9.81) -> np.ndarray:
    """
    Godunov精确Riemann通量
...
def evaluate_riemann_solution(sol: Dict, x: np.ndarray, t: float) -> Tuple[np.ndarray, np.ndarray]:
    """
    在给定位置和时间评估Riemann解
