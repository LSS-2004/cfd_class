...
PRESETS = {
    "标准溃坝 (B1默认)": {
        "domain_length": 1000.0, "nx": 100, "x_dam": 500.0,
        "h_l": 10.0, "h_r": 1.0, "u_l": 0.0, "u_r": 0.0,
        "g": 9.81, "t_end": 50.0, "cfl": 0.9,
        "boundary_type": "transmissive",
        "description": "B1 DamBreakConfig 默认值，大尺度溃坝"
    },
    "经典溃坝": {
        "domain_length": 10.0, "nx": 200, "x_dam": 5.0,
    ...
    "长时模拟": {
        "domain_length": 20.0, "nx": 400, "x_dam": 10.0,
        "h_l": 2.0, "h_r": 1.0, "u_l": 0.0, "u_r": 0.0,
    ...
    "细网格测试": {
        "domain_length": 10.0, "nx": 1000, "x_dam": 5.0,
        "h_l": 2.0, "h_r": 1.0, "u_l": 0.0, "u_r": 0.0,
        "g": 9.81, "t_end": 1.0, "cfl": 0.5,
        "boundary_type": "transmissive",
    ...
    with st.sidebar.expander("📐 域参数", expanded=True):
        ...
        with col1:
            domain_length = st.number_input(
                ...
                value=params.get("domain_length", 1000.0), step=100.0,
        ...
        with col2:
            nx = st.number_input(
                ...
                value=params.get("nx", 100), step=10,
                help="空间网格数量 (B1默认: 100)"
            )

        x_dam = st.slider(
            "大坝位置 [m]",
            min_value=0.0, max_value=domain_length,
            value=params.get("x_dam", domain_length / 2), step=10.0,
    ...
    with st.sidebar.expander("🌊 初始条件", expanded=True):
        ...
        with col1:
            ...
            h_l = st.number_input(
                ...
                value=params.get("h_l", 10.0), step=0.1,
    ...
    with st.sidebar.expander("⏱️ 时间参数", expanded=True):
        ...
        with col2:
            t_end = st.number_input(
                ...
                value=params.get("t_end", 50.0), step=1.0,
        ...
        with col3:
            cfl = st.number_input(
                ...
                value=params.get("cfl", 0.9), step=0.05,
